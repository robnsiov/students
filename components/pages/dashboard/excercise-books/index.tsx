import { Button } from "@/components/ui/button";
import { CheckCircle2, Minus, Plus } from "lucide-react";
import ExerciseBooksImpl from "./types";

const ExerciseBooks = ({ setState, state }: ExerciseBooksImpl) => {
  return (
    <>
      <div className="flex justify-start items-center flex-wrap mt-3">
        <span className="mr-4 mb-2">Exercise books</span>
        <div className="mr-4 flex justify-start items-center flex-wrap">
          <Button
            variant="default"
            className="mr-2 mb-2"
            onClick={() =>
              setState((prev) => ({ ...prev, task: "They're with me" }))
            }
          >
            They{"'"}re with me
            {state.task === "They're with me" && (
              <CheckCircle2 className="ml-2" size="18" />
            )}
          </Button>
          <Button
            className="mb-2"
            variant="default"
            onClick={() =>
              setState(() => ({ remains: 0, task: "They aren't with me" }))
            }
          >
            They aren{"'"}t with me
            {state.task === "They aren't with me" && (
              <CheckCircle2 className="ml-2" size="18" />
            )}
          </Button>
        </div>
        <div className="flex justify-start items-center flex-wrap">
          <code className="mr-2 mb-2">not done :</code>
          <div className="mb-2 flex justify-center items-center flex-wrap">
            <Button
              variant="outline"
              size="icon"
              onClick={() =>
                state.remains >= 90 ||
                setState((prev) => ({ ...prev, remains: prev.remains + 1 }))
              }
            >
              <Plus className="h-4 w-4" />
            </Button>
            <Button variant="outline" className="w-10 mx-2" size="icon">
              {state.remains}
            </Button>
            <Button
              variant="outline"
              size="icon"
              onClick={() =>
                state.remains <= 0 ||
                setState((prev) => ({ ...prev, remains: prev.remains - 1 }))
              }
            >
              <Minus className="h-4 w-4" />
            </Button>
          </div>
        </div>
      </div>
    </>
  );
};
export default ExerciseBooks;
